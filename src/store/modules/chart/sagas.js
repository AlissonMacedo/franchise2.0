...
export function* ticketAverageGetData() {
  try {
    const response = yield call(api.get, `avg-income?b=2020-10-01&e=2020-10-10`);

    const data = {
      Marketplace: {
        value: response.data.body.mktAvg,
        percent: response.data.body.mktGrowth,
      },
      Empresarial: {
        value: response.data.body.entAvg,
        percent: response.data.body.entGrowth,
      },
    };

    yield put(ticketAverageSuccess(data));
  } catch (err) {
...
export function* ordersAverageDayGetData() {
  try {
    const response = yield call(api.get, `avg-order-count?b=2020-10-01&e=2020-10-10`);
    ...
    yield put(ordersAverageDaySuccess(data));
  } catch (err) {
...
export function* gmvGetData() {
  try {
    const response = yield call(api.get, `gmv?b=2020-10-01&e=2020-10-30`);
    const ArrayDados = response.data.body.map(state => parseInt(state.gmv));
    const ArrayLabel = response.data.body.map(state => `${moment(state.date, 'yyyy:MM:DD').format('DD/MM')} `);
...
export function* ordersGetData() {
  try {
    const response = yield call(api.get, `order-count?b=2020-10-01&e=2020-10-10`);
    const ArrayDados = response.data.body.map(state => state.orderCount);
    const ArrayLabel = response.data.body.map(state => `${moment(state.date, 'yyyy:MM:DD').format('DD/MM')} `);
